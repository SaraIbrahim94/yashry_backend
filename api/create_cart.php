...
class cart extends database{
    ...
    public function create($conn)
    {
        ...
        else if($input['currancy'] == '' || $input['name'] == '')
        {
            echo "<span style='color:black; font-size:2rem'>Note:</span><p><strong> Please make sure you enterd all input values </strong></p>  <p>please enter input fields as specfied below.</p>";
            echo "<ul><li>product name with name as key</li> <li>currancy with currancy as key</li><p>ex: currancy=$&name[]=jacket&name[]=t-shirt</p>Full example: http://localhost/yashry_backend/api/create_cart.php?currancy=$&name[]=jacket&name[]=t-shirt<p></p></ul>";
        die;
    ...
    private function show_products($input, $conn)
    {
        $subtotal = 0;
        $discount_txt = [];
        $discount_total = 0;
                ...
                while($row = $products->fetch_assoc()) {
                    ...
                    $sql="SELECT * from currancy where product_id = '$row[id]' and currancy_type = '$input[currancy]'";
                        ...
                        if ($currancy) {
                            ...
                            $data["price"] = '<span style="color:red"> Please add price value and currancy to this product from this api: http://localhost/yashry_backend/api/add_currancy.php</span>';
                        ...
                        $tax = (((float)$data['price']*14)/100);
                        $subtotal += (float)$data['price'];
                        $price = $data['price'];
                        $data['price'] = (float)$data['price'] + $tax;
                        //get offer if exist
                        ...
                        if ($offer) {
                            ...
                            if ($offer->num_rows > 0) {
                                while($row_2 = $offer->fetch_assoc()) {
                                    ...
                                    {
                                        $amount = ((float)$data['price'] * (float)$row_2['discount'])/100;
                                        $discount = (float)$data['price'] - $amount;
                                        $discount_txt[] = (float)$row_2['discount'].'%  off '.$data['name']. ': -'. $amount .' '.$input["currancy"];
                                        $discount_total += $amount;
                                    }
